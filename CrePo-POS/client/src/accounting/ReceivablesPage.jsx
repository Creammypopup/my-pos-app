    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50 p-4">
            <div className="bg-white p-6 rounded-2xl shadow-lg w-full max-w-md">
                <h2 className="text-xl font-bold mb-2">รับชำระเงิน</h2>
                <p className="text-sm text-gray-500 mb-4">ใบเสร็จเลขที่: {sale.receiptNumber}</p>
                <div className="space-y-4">
                    <div>
                        <label className="block text-sm font-medium text-gray-700">ยอดค้างชำระ</label>
                        <p className="text-2xl font-bold text-red-600">{sale.balance.toLocaleString('th-TH', { style: 'currency', currency: 'THB' })}</p>
                    </div>
                    <div>
                 ...
                 <div className="flex justify-end space-x-4 pt-6 mt-4 border-t">
                    <button onClick={onClose} className="px-6 py-2 bg-gray-200 text-gray-800 rounded-lg">ยกเลิก</button>
                    <button onClick={handleSave} className="px-6 py-2 bg-green-500 text-white rounded-lg">บันทึกการชำระเงิน</button>
                </div>
            </div>
        </div>
    );
...
function ReceivablesPage() {
    ...
    return (
        <>
            <AddPaymentModal isOpen={isPaymentModalOpen} onClose={() => setIsPaymentModalOpen(false)} onSave={handleSavePayment} sale={selectedSale} />
